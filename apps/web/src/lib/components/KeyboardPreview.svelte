<script lang="ts">
	...
	import { effectColorOf, OFF_KEY_COLOR, type PreviewState } from '$lib/preview';
	...
	// Caps no browser event can ever report (hardware Fn keys, QMK combos) -
	// alias them so clicking paints them like any other key.
	const qmkAliases: Record<string, string> = {
		'MO(1)': 'FN',
		KC_GESC: 'ESC'
	};
	for (const [qmk, code] of Object.entries(qmkAliases)) {
		if (!qmkToMatrix[qmk]) qmkToMatrix[qmk] = code;
	}
	...
	function rgbFor(qmkCode: string): string {
		...
		if (preview === 'custom') {
			// unpainted keys stay at the faint baseline so legends remain readable
			return custom?.[matrixCode] ? dim(custom[matrixCode]) : OFF_KEY_COLOR;
		}
		const k = (matrixCode ? keyByCode.get(matrixCode) : null) ?? syntheticKeys.get(qmkCode);
		if (k && preview && preview !== 'off') return dim(effectColorOf(preview, effectParams, clock, k, pressed));
		// "off" keys skip the brightness dim so the key text stays visible
