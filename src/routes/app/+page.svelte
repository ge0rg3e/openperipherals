<script lang="ts">
	import { sessions, activeSessionId, error, addDevice, removeSession } from '$lib/store';
		...
		<header class="flex items-center justify-between gap-3">
			...
			<div class="flex items-center gap-2">
				<Button variant="outline" size="sm" onclick={() => addDevice()}>
					<Plus class="mr-1 h-3.5 w-3.5" /> Add Device
				</Button>
			</div>
		</header>
			...
			<div class="flex flex-col items-center gap-4 rounded-2xl border border-border/60 bg-gradient-to-b from-zinc-950 via-[#0a0c0e] to-black px-6 py-16 text-center shadow-2xl shadow-black/50">
				...
				<div class="flex flex-col gap-1">
					...
					<p class="max-w-md text-sm text-muted-foreground">
						Add a keyboard and a mouse to manage them side by side. Everything runs locally in your browser over WebHID.
					</p>
				</div>
				<div class="flex flex-wrap items-center justify-center gap-2">
					<Button size="sm" onclick={() => addDevice()}>
						<Plus class="mr-1 h-3.5 w-3.5" /> Add your first device
					</Button>
				</div>
			</div>
		{/if}
